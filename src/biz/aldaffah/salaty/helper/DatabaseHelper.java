package biz.aldaffah.salaty.helper;
...
import biz.aldaffah.salaty.parameters.Country;
...
public class DatabaseHelper extends SQLiteOpenHelper {
	private static String DB_PATH = "/data/data/biz.aldaffah.salaty/databases/";
	...
	public City getCity(long cityId) {
		...
		while (cur.isAfterLast() == false) {
			...
			// into a view object ( e.g. ListView )
			/*
			 * Here i have to set the city name to roman name and not arabic until i fix arabic language
			if (cur.getString(2) != null)
	...
	public Cursor getCityCursor(long id) {
		...
		Cursor cur = db
				.rawQuery(
						"SELECT cityNO as _id , cityName from citiesTable WHERE country_id = ? ORDER BY cityName ASC",
	...
	public Cursor getCountryList() {
		...
		Cursor cur = db
				.rawQuery(
						"SELECT country_id as _id , name as country_name from country ORDER BY name",
						null);
